...
if(!empty($aid)){
$rowEdit   = $funEventsObj -> find_by_id_type($aid);
}else{ 
$rf = $funEventsObj->table_fields(TABLE_EVENT_TYPE);		
if($rf){if($db->total_rows($rf)>0){while($rowfield=$db->fetch_array($rf)){@$rowEdit->{$rowfield['Field']}='';}}}
}//elseclose
?>
      <div class="panel-heading"> <span class="panel-title">Events Category <?php echo empty($aid)?" < <em class='add'>Add</em> > ":" < <em class='edit'>Edit</em> >"; ?></span> </div>
