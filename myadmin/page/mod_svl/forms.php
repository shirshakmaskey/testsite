...
<div class="panel-heading"> <span class="panel-title">
      ...
      <div class="panel-body">
              ...
              <form action="<?php echo base_url("$administrator/page/mod_$module/actions.php"); ?>" method="post" name="addEditForm" role="form" enctype="multipart/form-data" class="form-horizontal">    
          <input type="hidden" name="hidden_id" id="hidden_id" value="<?php echo @encode($aid); ?>" />
            ...
            <div class="form-group">
                ...
                <select name="artist_id" id="artist_id" class="form-control required">
        				...
        				<?php  $result  =  $funArtistObj->activeArtistAll();
							$num  =  $db->num_rows($result);
							if($num>0){
							while($row_artist  =  $db->result($result)){
		 				?>
        			<option value="<?php echo $row_artist->id?>" <?php echo (@$row->artist_id==$row_artist->id)?"selected":"";?>><?php echo $row_artist->artist_name?></option>
            			<?php }}?>
       			 </select>
                </div>     	       		
            </div>
                ...
                <select name="album_id" id="album_id" class="form-control required">
                     <option value="">Choose</option>
                        <?php 
                         ...
                         $result  =  $funAlbumObj->activeAlbumList($artist_id);
                            $num  =  $db->num_rows($result);
                            if($num>0){
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                	<div class="col-sm-offset-2 col-sm-10">
                        ...
                        <button type="button" class="btn btn-danger" id="cancel" value="Cancel" onclick="window.location.href='all-<?php echo $module;?>';">Cancel</button>
                    </div>
                </div>
        </form>
      </div>
...
$(function(){
   $("#artist_id").on("change",function(){
      var id = $(this).val();
      $.post(admin_url+'page/mod_svl/ajax.php',{id:id,mode:'list_album'},function(data){
      $("#album_id").html(data.html);
    },'json');
   });
});
</script>
