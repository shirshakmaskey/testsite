<?php
    $result  =  $funSVLObj->svlDataList();
?> 
<div class="page-header">
      
      <div class="row">
        <!-- Page header, center on small screens -->
        <h1 class="col-xs-12 col-sm-4 text-center text-left-sm"><i class="fa fa-tasks page-header-icon"></i>&nbsp;&nbsp;
        ...
        <div class="col-xs-12 col-sm-8">
          <div class="row">
            ...
            <a class="btn btn-primary btn-labeled" href="forms-<?php echo $module; ?>-<?php echo encode(0);?>"><span class="btn-label icon fa fa-plus"></span>Add New Songs or Video or Lyrics</a>

            <a class="btn btn-primary btn-labeled" href="lists-artist"><span class="btn-label icon fa fa-plus"></span>Add Album</a>

<a  href="lists-artist" class="btn btn-primary btn-labeled">
<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Artist List</a>
					...
					?>
                      <tr>
                        ...
                        <td><?php echo $row->title;?></td>
                         <td> Token : <?php echo $row->token_keys;?><br>
                          Artist:<?php $artist_id = $row->artist_id;
                          $artist_row = $funArtistObj->artist_detail($artist_id);?>
                          <a href="view-artist-<?php echo encode($artist_id);?>"><?php echo $artist_row->artist_name;?></a><br> 
                          Album : <?php $album_id  = $row->album_id;
                        $row_album = $funAlbumObj->album_detail($album_id);?>
                        <a href="view-album-<?php echo encode($album_id);?>"><?php echo @$row_album->album_name;?></a>
                         </td>
                        <td>
                        <?php $songs_file = $row->songs_file;?>
                       <audio controls>
                          <source src="<?php echo base_url('uploads/songs/'.$songs_file);?>" type="audio/mpeg">
                          Your browser does not support the audio tag.
                        </audio> 
                        <br>
                        <?php $video_file=  $row->video_file;
...
<iframe title="YouTube video player" class="youtube-player" type="text/html" width="100%" height="80" src="http://www.youtube.com/embed/<?php echo $video_code;?>" frameborder="0" allowFullScreen></iframe>
                        <br><a href="view-<?php echo $module?>-<?php echo @encode($row->id);?>">View Lyrics</a></td>
                        <td>
                        ...
                        <a href="forms-<?php echo $module?>-<?php echo encode($row->id);?>"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;
			 
             <a href="page/mod_<?php echo $module; ?>/actions.php?aid=<?php echo encode($row->id);?>&action=delete" onclick="return confirm('Are you sure you want to delete this row!!');" title="Delete"><span class="glyphicon glyphicon-trash"></span></a>
             </td>
                      </tr>
                      <?php $sn++;}
						   }
