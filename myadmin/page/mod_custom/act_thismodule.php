...
$funObj->table  =  TABLE_CUSTOM_BLOCKS;
...
{              
					...
					if(empty($hidden_id))
					{ $funObj->data['sortorder'] = $funCustomObj->find_maximum();
					$funObj->insert();
	  ...
	  $aid	=	$_GET['aid'];
	  $row  =  $funCustomObj->find_item_by_id($aid);
	  $img =  $row->image;
		if(file_exists(FCPATH.'uploads/images/custom/'.$img) and !empty($img)){ 
		 unlink(FCPATH.('uploads/images/custom/'.$img));
	  }//file exist
...
$funObj->url_back  = $sitePathB."list-custom";
