...
<?php
	...
	if(!empty($aid)){
	  	$row 	=  $funCustomObj->find_by_id($aid);
	}
	else{
	    $rf = $funNewsObj->table_fields();		
	...
	if(isset($_SESSION['imageFiles'])){ 
	  foreach($_SESSION['imageFiles'] as $key=>$val){
		 if(file_exists(FCPATH."uploads/images/custom/".$val) and !empty($val)){
			unlink(FCPATH."uploads/images/custom/".$val); 
		 } 
	  }
...
<div class="row">
  <div class="col-sm-12">
    <div class="panel">
      <div class="panel-heading"> <span class="panel-title">Manage Custom Blocks</span> </div>
      <div class="panel-body">
        <form action="<?php echo base_url("$administrator/page/mod_$module/act_thismodule.php"); ?>" method="post" enctype="multipart/form-data" id="addEditForm">
         ...
         <div class="form-group">
            <label class="col-sm-2 control-label" for="block_name">Block Name <span class="req">*</span></label>
         ...
         <div class="form-group">
            <label class="col-sm-2 control-label" for="block_title">Block Title <span class="req">*</span></label>
            <div class="col-sm-4"><input type="text" name="block_title" id="block_title" class="form-control required" value="<?php echo @$row->block_title;?>" /></div>
          </div>
        
         <div class="form-group">
            <label class="col-sm-2 control-label" for="position">Position <span class="req">*</span></label>
           <div class="col-sm-4"> <input type="radio" value="left" name="position" <?php echo @($row->position=='left')?"checked":"";?> />
            &nbsp;Left
   ...
   if($aid>0){
			    $img =  $row->image;
				if(file_exists(FCPATH.'uploads/images/custom/'.$img) and !empty($img)){ ?>
              <div class="thumb_box" id="thumb_image_<?php echo $row_item->id; ?>"><a href=""><img src="<?php echo base_url(APPLICATIONS);?>timthumb/timthumb.php?src=<?php echo base_url('uploads/images/custom/'.$img); ?>&w=200&h=100" class="img-polaroid"></a>
  ...
  ?>
         ...
         <div class="form-group">
            ...
            <div class="col-sm-4">
            <input type="radio" name="link_type" value="internal" <?php echo @($row->url_link=="internal")?"checked":"";?> />
            Internal
...
<script type="text/javascript">
   // <![CDATA[
	...
	'formData'    	: {project_path : '<?php echo base64_encode(constant("FCPATH")); ?>',				                       targetFolder:'uploads/images/custom/'},
	'method'   		: 'post',
...
function deleteImage(id)
{
	if(confirm('Are you sure, you want to delete this?')){
		  $.post(admin_url+'page/mod_custom/ajax.php',{id:id,mode:'delete_image'},function(data){
