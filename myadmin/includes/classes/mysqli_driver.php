 ...
 */
 ...
 { if (array_key_exists($index, $this->vars)) {
	...
	{
		return $this->conn_id = @mysqli_connect(constant("HOST"),constant("USER"),constant("PASS"),constant("DBNAME")) or  die("Failed Connecting To Mysql<br>".mysqli_error());
	}
   ...
   {   if($debug){ die($this->query); }  
	...
	function get($table="",$fields="",$cond="",$order_by="",$lim=0)
		{   
			...
			if($this->condition=='' or (count($this->condition)==0)) $this->condition = "";
