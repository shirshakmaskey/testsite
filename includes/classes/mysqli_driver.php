 ...
 */
 ...
 {  
	...
	$this->lim        =  FALSE;
	$this->query      =  FALSE;
	$this->conn_id = @mysqli_connect(constant("HOST"),constant("USER"),constant("PASS"),constant("DBNAME")) or  die("Failed Connecting To Mysql<br>".mysql_error());	
 }
 ...
 */
 ...
 {  
	...
	{  
		return $this->conn_id = @mysqli_connect(constant("HOST"),constant("USER"),constant("PASS"),constant("DBNAME")) or  die("Failed Connecting To Mysql<br>".mysql_error());
	}
	...
	{   if(empty($sql)) { die("Query is Empty"); }
		...
		$this->result = mysqli_query( $this->conn_id,$this->query ) or die("error_occur here at exec"
										 ."<li>errorno=".mysqli_errno($this->conn_id)
										 ."<li>error=".mysqli_error($this->conn_id)
										 ."<li>query=".$this->query
										 );
   ...
   {   if($debug){ die($this->query); }  
	...
	{   
		...
		if(is_array($this->condtion) and count($this->condtion)==0){$this->condition = "";}          		 
	...
	function seo()
	{
	   $sql = "select * from tbl_seo where id='1'";	
