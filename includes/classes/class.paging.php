...
class paging {
	...
	function paging($seo=0, $first="First", $last="Last", $next="»", $previous="«"){
	...
	//$="";
		...
		{		
			...
			if($this->pPageID!==1)
			{
				...
				if($this->seo_url == 0){
					$link .= "<li><a href=\"$url".((!strpos($url,"?"))?"?":"&")."np=1\" class=\"$cssclass\"> <strong>".$this->paging_first." </strong></a></li>";
					$link .= "<li><a href=\"$url".((!strpos($url,"?"))?"?":"&")."np=$prevPage\" class=\"$cssclass\">".$this->paging_previous."</a></li>";
				}
				else{
					$link .= "<li><a href=\"".$url."/1\" class=\"$cssclass\"> <strong>".$this->paging_first." </strong></a></li>";
					$link .= "<li><a href=\"".$url."/".$prevPage."\" class=\"$cssclass\">".$this->paging_previous."</a></li>";
			...
			{	
				...
				if($this->seo_url == 0){
					if($this->pPageID==$ctr)
					$link .=  "<li><a class='active'>$ctr</a></li>";
					else
					$link .= "<li><a href=\"$url&np=$ctr\" class='add_menus'>$ctr</a></li>";
				}
				else{
					if($this->pPageID==$ctr)
					$link .=  "<li><a class='active'>$ctr</a></li>";
					else
					$link .= "<li><a href=\"".$url."/".$ctr."\" class='add_menus'>$ctr</a></li>";
				}
			}
			//Previous Next link
			if($this->pPageID<($ctr-1))
			{
				...
				if($this->seo_url == 0){
					$link .= "<li><a href=\"$url".((!strpos($url,"?"))?"?":"&")."np=$nextPage\" class=\"$cssclass\"> ".$this->paging_next." </a></li>";	
					$link .="<li><a href=\"$url".((!strpos($url,"?"))?"?":"&")."np=".$this->pRecord."\" class=\"$cssclass\"> <strong>".$this->paging_last."</strong> </a></li>";
				}
				else{
					$link .= "<li><a href=\"".$url."/".$nextPage."\" class=\"$cssclass\"> ".$this->paging_next." </a></li>";	
					$link .="<li><a href=\"".$url."/".$this->pRecord."\" class=\"$cssclass\"> <strong>".$this->paging_last."</strong> </a></li>";
				}
			}
			return $link."</ul>";
		}			
