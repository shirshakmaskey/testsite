...
{  public $action;
   ...
   public $url_back;
		...
		{    $action = $this->action; 
			...
			$_SESSION['succesMesage']="Data has been added sucessfully!!";
			return $queryResult;
			}
			...
			$_SESSION['succesMesage']="Data has been edited sucessfully!!";			
			return $queryResult;
			}
			...
			$_SESSION['succesMesage']="Data has been deleted sucessfully!!";
			return $queryResult;
			}						
		}
	...
	* @return 1 or null
	*/
	function sendEmail($fromName='',$fromEmail,$receiverName='',$receiverEmail, $subject, $content, $cc='',$bcc='',  $attachment=array(), $debug=false) {
	
		$sendMail = new PHPMailer();
		if ($fromName != '') {
			$sendMail->addReplyTo($fromEmail, $fromName);
			$sendMail->setFrom($fromEmail,$fromName);	
	    }else{
            $sendMail->addReplyTo($fromEmail);	
			$sendMail->setFrom($fromEmail,$fromName);
	    }

	    if ($receiverName != '') {
			$sendMail->addAddress($receiverEmail, $receiverName);	
	    }else{
		...
		$sendMail->msgHTML(html_entity_decode($content));
        if(count($attachment)>0){ 
           foreach($attachment as $key=>$val){ 
           	  if(count($val)==2) $sendMail->addAttachment($val[0], $val[1]);
           	  	else $sendMail->addAttachment($val[0]);
           }
        }
	...
	*/
	...
	public function ConfigValue($configname)
	{
	 ...
	 public function configRow($configname)
	{
		$sql = "SELECT * FROM `".TABLE_CONFIGURATION."` where `configname`='$configname' and status='1'";
		$result  =   $this->exec($sql);
	...
	/************************************ Modules  users *****************************************/
	 ...
	 {
		 $sql = "SELECT * FROM `".TABLE_COUNTRY."`  order by id asc";
		return  $this->exec($sql); 
    ...
    public function modulesList()
	 ...
	 {  
	       // code here
		    $accesiblePage  =  array();
			$accesibleMod  =  array();
			$resultModules  =  $this->modulesListPermisson($_SESSION['pathsaleLoginId'],$_SESSION[ENCR_KEY.'level'],0); 
			...
			{    ////////////////////////////////////////
				...
				$resultModulesSub  =  $this->modulesListPermisson($_SESSION['pathsaleLoginId'],$_SESSION[ENCR_KEY.'level'],$rowModules->id);
...
include(FCPATH."includes/helpers.php");	
?> 