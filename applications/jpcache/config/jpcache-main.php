<?php
    ...
    function jpcache_varkey()
    {
        ...
        // serialize get values minus the flush parameter
        $get = array();
        $get = array_diff($_GET, array('flush' => $GLOBALS["JPCACHE_FLUSH_KEY"]));
    ...
    function jpcache_scriptkey()
    {
        ...
        {
            $name = isset($_SERVER["SCRIPT_URI"]) ? $_SERVER["SCRIPT_URI"] : '';
        }
    ...
    function jpcache_check()
    {
        ...
        jpcache_debug("Looking for cachedata for ".$GLOBALS["jpcache_key"]);
        if (jpcache_restore())
        {
    ...
    function jpcache_init()
    {
        ...
        header("X-Cache: jpcache ".$GLOBALS["JPCACHE_VERSION"].
    ...
    function jpcache_start()
    {
        ...
        // Check cache
        if (jpcache_check() && !(isset($_GET['flush']) && $_GET['flush'] == $GLOBALS["JPCACHE_FLUSH_KEY"]))
        {
        ...
        {  
			// flush a single page
			if (isset($_GET['flush']) && isset($GLOBALS["jpcache_key"]) && $_GET['flush'] == $GLOBALS["JPCACHE_FLUSH_KEY"])
			{
   		    	jpcache_do_gc('FLUSHSINGLE');
   		    	jpcache_debug("Flushed cachedata for ".$GLOBALS["jpcache_key"]);
			}
			else
			{  
			...
			// if we came here, cache is invalid (or we invalidated it): go generate page
			// and wait for jpCacheEnd() which will be called automagically

			// Go generate page and wait for callback
			$tmp = (isset($GLOBALS['jpcache_key'])) ? $GLOBALS['jpcache_key'] : 'NULL';
			jpcache_debug("Cachedata not found for ".$tmp." key, generating now");
                ...
                if ($ENCODING)
                {
                   ...
                   $ret =  "\x1f\x8b\x08\x00\x00\x00\x00\x00";
                    ...
                    header("Content-Encoding: $ENCODING");
						  header('Vary: Accept-Encoding'); //Required to prevent potential cache problems!
						  header('Content-Length: ' . strlen($ret));
						  header('X-Content-Encoded-By: jpcache ' . $GLOBALS["JPCACHE_VERSION"] . ' - ' . $GLOBALS["JPCACHE_TYPE"]);
                 }
                else
                {
