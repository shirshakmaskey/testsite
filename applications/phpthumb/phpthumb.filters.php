		...
		if (phpthumb_functions::version_compare_replacement(phpversion(), '4.3.2', '>=')) {
			...
			if ($gdimg_mask_resized = phpthumb_functions::ImageCreateFunction(ImageSX($gdimg_image), ImageSY($gdimg_image))) {
				...
				if ($gdimg_mask_blendtemp = phpthumb_functions::ImageCreateFunction(ImageSX($gdimg_image), ImageSY($gdimg_image))) {
					...
					ImageCopy($gdimg_mask_blendtemp, $gdimg_image, 0, 0, 0, 0, ImageSX($gdimg_image), ImageSY($gdimg_image));

					$color_background = ImageColorAllocate($gdimg_image, 0, 0, 0);
					ImageFilledRectangle($gdimg_image, 0, 0, ImageSX($gdimg_image), ImageSY($gdimg_image), $color_background);
					//ImageColorTransparent($gdimg_image, $color_background);
					...
					for ($x = 0; $x < ImageSX($gdimg_image); $x++) {
						for ($y = 0; $y < ImageSY($gdimg_image); $y++) {
							...
							$newcolor = phpthumb_functions::ImageColorAllocateAlphaSafe($gdimg_image, $RealPixel['red'], $RealPixel['green'], $RealPixel['blue'], $MaskAlpha);
							ImageSetPixel($gdimg_image, $x, $y, $newcolor);
						}
					}
...
//						ImageFill($gdimg_tiledwatermark, 0, 0, ImageColorTransparent($gdimg_tiledwatermark));
						...
						for ($x = round((1 - $watermark_margin_percent) * $img_source_width); $x < ($img_source_width + $watermark_source_width); $x += round($watermark_source_width + ((1 - $watermark_margin_percent) * $img_source_width))) {
							for ($y = round((1 - $watermark_margin_percent) * $img_source_height); $y < ($img_source_height + $watermark_source_height); $y += round($watermark_source_height + ((1 - $watermark_margin_percent) * $img_source_height))) {
								ImageCopy(
				...
				case 'T':
					$watermark_destination_x = round((($img_source_width  / 2) - ($watermark_source_width / 2)) + $watermark_margin_percent);
					$watermark_destination_y = round((1 - $watermark_margin_percent) * $img_source_height);
					break;
				...
				case 'L':
					$watermark_destination_x = round((1 - $watermark_margin_percent) * $img_source_width);
					$watermark_destination_y = round((($img_source_height / 2) - ($watermark_source_height / 2)) + $watermark_margin_percent);
					break;
