...
class phpthumb {
	...
	var $phpthumb_version = '1.5.0-200502041152';
	...
	function OutputThumbnail() {
		if ($this->RemoveFileOnCompletion) {
			$this->DebugMessage('deleting $this->sourceFilename ('.$this->sourceFilename.')', __FILE__, __LINE__);
			@unlink($this->sourceFilename);
		}
	...
	function ImageMagickCommandlineBase() {
		...
		if (is_null($commandline)) {
			...
			if ($this->config_imagemagick_path != realpath($this->config_imagemagick_path)) {
			...
			} elseif ($which_convert && ($which_convert{0} == '/')) {
	...
	function ImageMagickThumbnailToGD() {
		...
		$this->DebugMessage('ImageMagickThumbnailToGD() aborting', __FILE__, __LINE__);
		return false;
	}
	...
	function AntiOffsiteLinking() {
		...
		$allow = true;
		if ($this->config_nooffsitelink_enabled && !empty($_SERVER['HTTP_REFERER'])) {
		...
		if ($allow && $this->config_nohotlink_enabled && (substr(strtolower($this->src), 0, 7) == 'http://')) {
		...
		if ($erase) {
			...
			return $this->ErrorImage($message, $this->thumbnail_width, $this->thumbnail_height, $this->config_nohotlink_fill_hexcolor, $this->config_nohotlink_text_hexcolor, $this->config_nohotlink_text_fontsize);

		} else {

			$nohotlink_text_array = explode("\n", wordwrap($message, floor($this->thumbnail_width / ImageFontWidth($this->config_nohotlink_text_fontsize)), "\n"));
			$nohotlink_text_color = phpthumb_functions::ImageHexColorAllocate($this->gdimg_output, $this->config_nohotlink_text_hexcolor);

			$topoffset = round(($this->thumbnail_height - (count($nohotlink_text_array) * ImageFontHeight($this->config_nohotlink_text_fontsize))) / 2);

			$rowcounter = 0;
			foreach ($nohotlink_text_array as $textline) {
				$leftoffset = max(0, round(($this->thumbnail_width - (strlen($textline) * ImageFontWidth($this->config_nohotlink_text_fontsize))) / 2));
				ImageString($this->gdimg_output, $this->config_nohotlink_text_fontsize, $leftoffset, $topoffset + ($rowcounter++ * ImageFontHeight($this->config_nohotlink_text_fontsize)), $textline, $nohotlink_text_color);
			}
	...
	function AlphaChannelFlatten() {
		...
		if ($this->thumbnailFormat == 'png') {
			...
			$this->DebugMessage('skipping AlphaChannelFlatten() because ($this->thumbnailFormat == "png")', __FILE__, __LINE__);
			return false;
		}
	...
	function SetCacheFilename() {
		...
		if (!empty($_SERVER['HTTP_REFERER'])) {
			...
			$parsed_url2 = @parse_url(@$_SERVER['HTTP_HOST']);
			if (@$parsed_url1['host'] && @$parsed_url2['host'] && ($parsed_url1['host'] != $parsed_url2['host'])) {
				...
				$this->cache_filename .= '_httpreferer'.@$parsed_url1['host'];
			}
		}
		if (!empty($_REQUEST['fltr']) && is_array($_REQUEST['fltr'])) {
			$this->cache_filename .= '_fltr'.str_replace('|', '¦', implode('_fltr', $_REQUEST['fltr']));
		}
		...
		if (empty($this->gdimg_source)) {
			...
			if (empty($this->gdimg_source)) {
				...
				if ($this->ImageMagickThumbnailToGD()) {
					...
					$this->DebugMessage('ImageMagickThumbnailToGD() failed, outputting unmodified original image data', __FILE__, __LINE__);
	...
	function phpThumbDebug() {
		...
		$ParameterNames     = array('src', 'w', 'h', 'f', 'q', 'sx', 'sy', 'sw', 'sh', 'far', 'bg', 'bgt', 'bc', 'file', 'goto', 'err', 'xto', 'ra', 'ar', 'aoe', 'iar', 'maxb');
